...
export const Scheduler: React.FC<SchedulerProps> = ({ 
  ...
  const handleOpenModal = (schedule?: SyncSchedule) => {
      if (schedule) {
          ...
          setTargetFolder('/Finance/Statements');
  ...
  const handleOpenFolderPicker = (e: React.MouseEvent) => {
      e.preventDefault(); 
      if (storageType === 'GoogleDrive') {
        setCurrentPath(['My Drive']);
        setIsFolderPickerOpen(true);
      } else {
          // Simulate local folder picker
          setTargetFolder('C:\\Users\\User\\Downloads\\BankStatements');
      }
  };
                        ...
                        <div>
                            ...
                            <div className="flex gap-2">
                                <div className="flex-1 bg-slate-900 border border-slate-700 rounded-lg px-4 py-2.5 text-slate-300 font-mono text-sm truncate flex items-center">
                                    {targetFolder || <span className="text-slate-600 italic">{storageType === 'GoogleDrive' ? 'No folder selected' : 'Default Downloads Folder'}</span>}
                                </div>
                                <button 
                                    type="button"
